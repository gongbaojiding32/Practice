...
function drawTable(data) {
    ...
    if (parr == 1 && rarr > 1) {
        for (let j = 0; j < data.length; j++) {
            if (j == 0) {
                str = str + '<tr><td class="rows" rowspan="' + data.length + '">' + data[j].product + '</td><td>' + data[j].region + '</td>';
            } else {
        ...
        for (let j = 0; j < data.length; j++) {
            if (j == 0) {
                str = str + '<tr><td class="rows" rowspan="' + data.length + '">' + data[j].region + '</td><td>' + data[j].product + '</td>';
            } else {
        ...
        for (let j = 0; j < data.length; j++) {
            if (data[j].count) {
                str = str + '<tr><td class="rows" rowspan="' + data[j].count + '">' + data[j].product + '</td><td>' + data[j].region + '</td>';
            } else {
                str = str + '<tr><td>' + data[j].region + '</td>';
            }
            for (let k = 0; k < data[j].sale.length; k++) {
